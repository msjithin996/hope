...
FACE_COLLECTION = "faceapp_threat"
...
post '/compare' do
  ...
  if response.face_matches.count > 1
    ...
    {:id => response.face_matches[0].face.external_image_id, :confidence => response.face_matches[0].face.confidence, :message => "Threat found!"}.to_json
  end
end
